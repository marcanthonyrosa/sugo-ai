...
| **B-46** | **The side margin is the measure.** Document setup is 0.85 in top · **1 in sides** · 0.8 in bottom, body 10.5 pt on 1.5. Because nothing caps its own width, the margin is the only thing setting line length. Measure the built PDF, never estimate: ideal 75–85 characters, acceptable to 90, long past that. **Sugo documents run wide by choice, at ~90.** Sides match the head, so the page carries one frame value rather than two tuned ones, and 0.9 in clears a three-hole punch where 0.75 in does not. *Amended twice on 28 Aug 2026: 1.5 in measured a comfortable 73 but read as a book beside a state filing; 1 in measured 87. Settled at **0.9 in**, the middle of the range Marc bracketed, which measures 90 — exactly the median of the SOS / IRS letter used as the benchmark. The evidence that made this an easy call: on the same report the original typography at 0.75 in ran 90 characters and the rebuilt typography at 0.75 in runs 92, yet one reads far better than the other — **the margin was never what made the first draft hard to read.** B-47 to B-51 did that work. **Margin barely moves the measure and does not move the page count at all.** Across 0.75 in to 1 in the count runs 92 down to 87, and 0.75 / 0.8 / 0.85 / 0.875 / 0.9 / 1 in all set the same report in nine. Choose the side margin on how the page looks and prints, not on line length.* | The first draft ran a median 98 characters a line, p90 106, against a best-practice ceiling of 75. That one number produced most of the "hard to read" symptoms, including the impression that the type was too large — type that feels oversized is usually type oversized *for its measure*. Widening these margins is a typographic decision, not a whitespace one |
...
| **B-54** | **Briefing front matter is two lines.** Lockup and title on one row, closed by the 2 px ink rule; one body-face byline beneath: prepared for, author, date, research as-of. No parties band, meta run or brand eyebrow. The legal entity appears once, in the closing block | On the first long-form client briefing (11 Sep 2026) the header repeated the date twice and the company name three times, carried six label runs and three rules, and took ~19% of page one before the first sentence. Marc: the header and intro "feel like so much noise… a lot and at times duplicative" |
| **B-55** | **Briefing labels are sentence case in the body face.** Tracked mono capitals only on table column headers and chart axes, at **0.06 em**. Mono for codes, record numbers, figures and dates; footnotes and the legal block in the body face | The briefing carried 49 tracked-cap labels in six pages. At 0.10–0.14 em, text copied out of the PDF broke apart (`1 1 SE P 20 2 6`, "PREPARE D FOR"), which also fails search and screen readers |
| **B-56** | **One frame per document.** The 2 px ink border goes on at most one object, and only one that depicts a real artifact; nothing nests inside it but rows. Column, step and figure rows sit open under a hairline, with no seams | Seven framed boxes in one briefing, one of them four layers deep (frame, tag, tinted panel, table). A Hallmark audit returned card-in-card and boxed three-column rows as its two critical findings |
| **B-57** | **A fill or a rule, never both.** At most one filled box per briefing (plain terms, `viola`, no border). Notes and limitations are open text under a hairline. No ink flood in a briefing | Every callout carried a 2 px border and a fill: emphasis applied twice on three kinds of box, so none of them read as the one that mattered |
| **B-58** | **Briefing tables carry two tiers of fill.** Column headers on `viola-50` with a hairline beneath; group rows on `paper-2`, body face 600, padded like any other row. No column that repeats one word down the page. A tint means one thing per table | Stripping every fill left tables with no entry point; Marc asked for "some kind of fill… enough to help introduce the content". Group rows first shipped with extra space above and read too tall. A status column said the same word in 9 of 11 rows, and one table gave two opposite rows the same tint |
| **B-59** | **Footnotes are a compact key:** body face 7.5 pt on 1.35, 1.5 pt apart, 6 pt above the block | Five mono footnotes at 8.5 pt read as a second body; Marc asked for "smaller profile with less spacing and more condensed" |
| **B-60** | **Headline figures sit in an open row** under a hairline, mono 17 pt, and never restate the lede above them | Boxed stat tiles repeated the sentence directly above them |
| **B-61** | **Briefing charts are positioned markup, not script.** Navy and violet only, saffron at most once; one glyph set (`g-diamond`, `g-square`, `g-square-open`, `g-dot`, `g-ring`, `g-tri`, `g-tri-alt`, `g-circle-open`, `g-dashed`); `role="img"` with a one-sentence label; nothing under 7 pt; marks at 3:1 or better | WeasyPrint runs no JavaScript. The audit found `viola-500` marks at ~2.6:1, 6 pt chart text, and hundreds of positioned spans with no text alternative |
| **B-62** | **Copy states content, not the document.** No roadmap sentences, no section summaries by number, no headings that describe the page. Illustrative data is labelled once, inline, as a bold lead-in | Marc, on the same briefing: "There's some meta narration we can purge, like 'Sections 1 and 4 are real records… Sections 2 and 3 use an illustrative buyer…'" |
| **B-63** | **Check the rendered page, not the markup.** Render and look at every page; shorten a jumping block rather than force a break; copy text out with `pdftotext` to catch tracking; count the devices per document and keep only those that carry information the words do not | Every defect in B-54 to B-62 was visible only in the rendered PDF. Forced page breaks produced three near-empty pages, and a chart moved whole and stranded its heading |
...
| **C-11** | **Do B-54, B-55 and B-57 extend beyond briefings?** Two-line front matter, 0.06 em tracking and fill-or-rule would quiet proposals and reports too; invoices and SOWs still need the parties band | The copy-paste breakage behind B-55 affects every document that uses the 0.14 em eyebrow today |
